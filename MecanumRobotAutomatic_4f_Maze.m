...
if(connected)
    ...
    points={[-1.05535e-4,-2.6822e-07,0],[-6.7512e-01,0,0],[-6.7513e-01,-1.9751,0],[-2.2252,-2,0],[-2.2503,3.4996e-01,0],[-1.3755,1.7249,0]};
  ...
  while abs(destination(1,1)-currentPosition(1,1))>epsilon || abs(destination(1,2)-currentPosition(1,2))>epsilon
       ...
       % Vtheta = (destination(1,3) - currentPosition(1,3))*step(positionErrorh);
        %Vtheta = (theta - currentOrientation(1,3)* pi/180)*step(positionErrorh);
